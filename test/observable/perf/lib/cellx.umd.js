(function (global, factory) {
  ...
  class Cell extends EventEmitter {
    constructor(value, options) {
      ...
      this._pull = (options && options.pull) || (typeof value == "function" ? value : null);
  ...
  class ObservableList extends EventEmitter {
      ...
      return new this.constructor(
        deep
          ? this._items.map((item) =>
              item && typeof item == "object" && item.clone
                ? item.clone.length
                  ? item.clone(true)
                  : item.clone()
                : item,
    ...
    absorbFrom(that) {
      ...
      for (let i = items.length; i; ) {
        ...
        if (item !== thatItem) {
          if (item && thatItem && item.absorbFrom && item.absorbFrom === thatItem.absorbFrom) {
