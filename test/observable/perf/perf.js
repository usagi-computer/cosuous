...
test("one observes ten thousand that observe one", function (t) {
  ...
  subscribe(() => b()); // start observers
  // mobx.observe(b, voidObserver, true);
  t.equal(99990000, b());
  const initial = now();
...
test("five hundrend properties that observe their sibling", function (t) {
  ...
  subscribe(() => last()); // start observers
...
test("observe and dispose", (t) => {
  ...
  for (let i = 0; i < MAX * 2; i++) observers.push(subscribe(() => a()));
  const afterSubscribe = now();
...
//     computeds.push(
//       computed(() =>
//         i ? computeds[i - 1]() + computeds[i - 1]() : 1
//       )
//     );
//   }
//   const start = now();
