...
test("two boolean props", () => {
  expect(html` <a invisible disabled /> `).toEqual({
    tag: "a",
...
test("two props with empty values", () => {
  expect(html` <a href="" foo="" /> `).toEqual({
    tag: "a",
    props: { href: "", foo: "" },
    children: [],
  });
...
test("single prop with empty name", () => {
  expect(html`
      <a ""="foo" />
    `).toEqual({ tag: "a", props: { "": "foo" }, children: [] });
});

test("single prop with static value", () => {
  expect(html` <a href="/hello" /> `).toEqual({
    tag: "a",
    props: { href: "/hello" },
    children: [],
  });
...
test("single prop with static value followed by a single boolean prop", () => {
  expect(html` <a href="/hello" b /> `).toEqual({
    tag: "a",
...
test("two props with static values", () => {
  expect(html` <a href="/hello" target="_blank" /> `).toEqual({
    tag: "a",
...
test("slash in the middle of tag name or property name self-closes the element", () => {
  expect(html`
      <ab/ba prop=value>
    `).toEqual({ tag: "ab", props: null, children: [] });
  expect(html`
      <abba pr/op=value>
    `).toEqual({ tag: "abba", props: { pr: true }, children: [] });
});

test("slash in a property value does not self-closes the element, unless followed by >", () => {
  expect(html`<abba prop=val/ue><//>`).toEqual({
    tag: "abba",
    props: { prop: "val/ue" },
    children: [],
  });
  expect(html` <abba prop="value" /> `).toEqual({
    tag: "abba",
    props: { prop: "value" },
    children: [],
  });
...
test("two props with dynamic values", () => {
  function onClick() {}
  expect(html` <a href=${"foo"} onClick=${onClick} /> `).toEqual({
    tag: "a",
...
test("prop with multiple static and dynamic values get concatenated as strings", () => {
  ...
  expect(html`
    ...
    `).toEqual({ tag: "a", props: { href: "/before/foo/after" }, children: [] });
  expect(html`
      <a href=/before/${"foo"}/>
    `).toEqual({ tag: "a", props: { href: "/before/foo" }, children: [] });
});
...
test("spread props", () => {
  expect(html` <a ...${{ foo: "bar" }} /> `).toEqual({
    tag: "a",
    props: { foo: "bar" },
    children: [],
  });
  expect(html` <a b ...${{ foo: "bar" }} /> `).toEqual({
    tag: "a",
    props: { b: true, foo: "bar" },
    children: [],
  });
  expect(html` <a b c ...${{ foo: "bar" }} /> `).toEqual({
    tag: "a",
    props: { b: true, c: true, foo: "bar" },
    children: [],
  });
  expect(html` <a ...${{ foo: "bar" }} b /> `).toEqual({
    tag: "a",
    props: { b: true, foo: "bar" },
    children: [],
  });
  expect(html` <a b="1" ...${{ foo: "bar" }} /> `).toEqual({
    tag: "a",
    props: { b: "1", foo: "bar" },
    children: [],
  });
  expect(html` <a x="1"><b y="2" ...${{ c: "bar" }} /></a> `).toEqual(
    h("a", { x: "1" }, h("b", { y: "2", c: "bar" })),
  );
  expect(html` <a b=${2} ...${{ c: 3 }}>d: ${4}</a> `).toEqual(h("a", { b: 2, c: 3 }, "d: ", 4));
  expect(html` <a ...${{ c: "bar" }}><b ...${{ d: "baz" }} /></a> `).toEqual(
    h("a", { c: "bar" }, h("b", { d: "baz" })),
  );
});

test("multiple spread props in one element", () => {
  expect(html` <a ...${{ foo: "bar" }} ...${{ quux: "baz" }} /> `).toEqual({
    tag: "a",
...
test("mixed spread + static props", () => {
  ...
  expect(html` <a ...${{ foo: "bar" }} b c /> `).toEqual({
    tag: "a",
    props: { b: true, c: true, foo: "bar" },
    children: [],
  });
...
test("non-element roots", () => {
  expect(html` foo `).toEqual(h(["foo"]));
  expect(html` ${1} `).toEqual(h([1]));
  expect(html` foo${1} `).toEqual(h(["foo", 1]));
  expect(html` foo${1}bar `).toEqual(h(["foo", 1, "bar"]));
});
...
test("mixed text + dynamic children", () => {
  expect(html` <a>${"foo"}bar</a> `).toEqual({ tag: "a", props: null, children: ["foo", "bar"] });
  expect(html` <a>before${"foo"}after</a> `).toEqual({
    tag: "a",
    props: null,
...
test("multiple element children", () => {
  ...
  expect(html` <a x><b y /><c z /></a> `).toEqual(
    h("a", { x: true }, h("b", { y: true }), h("c", { z: true })),
  );
  expect(html` <a x="1"><b y="2" /><c z="3" /></a> `).toEqual(
...
test("mixed typed children", () => {
  ...
  expect(html` <a>before<b x="1" />after</a> `).toEqual(
...
test("NUL characters are allowed in attribute values", () => {
  expect(html` <a b=" "></a> `).toEqual(h("a", { b: "\0" }));
  expect(html` <a b=" " c=${"foo"}></a> `).toEqual(h("a", { b: "\0", c: "foo" }));
});

test("NUL characters are allowed in text", () => {
  expect(html` <a></a> `).toEqual(h("a", null, "\0"));
  expect(html`
      <a>\0${"foo"}</a>
    `).toEqual(h("a", null, "\0", "foo"));
});

test("cache key should be unique", () => {
  html` <a b="${"foo"}" /> `;
  expect(html` <a b=" " /> `).toEqual(h("a", { b: "\0" }));
